...
export default function RevenueStats({ scans }) {
  // Only count original transactions (not undone, not reversals)
  const activeValid = scans.filter((s) => !s.isUndone && !s.isReversal && s.scanResult === 'valid');
  ...
  return (
    <div className="grid grid-cols-3 gap-3 mb-2">
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-center">
         ...
         <p className="text-emerald-400 font-bold text-lg leading-none">CAD ${totalRevenue.toFixed(2)}</p>
       ...
       <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-center">
         ...
         <p className="text-purple-400 font-bold text-lg leading-none">CAD ${avgSpend.toFixed(2)}</p>
